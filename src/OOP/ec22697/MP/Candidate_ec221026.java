...
class Candidate_ec221026 extends Candidate {
   ...
   // If no candidates in list, vote for myself
       if (candidates.length == 0) {
           return new Candidate_ec221026();
       }


       for (Candidate c : candidates) {
           if (c.getName().equals("Daniel")) {
               return c;
           }
       }
   ...
   // Search for a condidate with the slogan "More Sleep!"
    ...
    public static void main (String [] args){
        ...
        Candidate [] all_votes = new Candidate [all_Candidates.length];
        int numOfCandidates = 0;
        String inp = inputString ("Would you like to :  a) Add specific candidate  b) run the election");
         while (!inp.equals("a") && !inp.equals("b")){
            System.out.println("input not recognised please use a valid input");
            inp = inputString ("Would you like to :  a) Add specific candidate  b) run the election");
        }
        if (inp.equals("a")){
            String name = inputString("Please enter a username.");
            Candidate newCandidate = A3.getByUsername(name, all_Candidates);
            all_votes[numOfCandidates] = newCandidate;
            numOfCandidates++;

            candidateList(all_votes, numOfCandidates);
        }
        else if (inp.equals("c")){
            String whoCount = inputString("Who should count the votes?");
            Candidate c = A3.getByUsername(whoCount, all_Candidates);
            while (c == null)
            {
                System.out.println("This candidate doesn't exist, try another candidate");
                whoCount = inputString("Who should count the votes?");
                c = A3.getByUsername(whoCount, all_Candidates);
            }
            System.out.println("How many times shall we run the election?");
            int numtimes = Integer.parseInt(scanner.nextLine());
            runElection(all_votes, numOfCandidates, numtimes, c, all_Candidates);
        }
    }
    ...
    public static void runElection(Candidate[] all_votes, int numOfCandidates, int numtimes, Candidate c, Candidate[] all_Candidates){
        Candidate[] election = new Candidate[numOfCandidates];
        for (int i = 0; i < numOfCandidates; i++) {
            election[i] = all_votes[numOfCandidates];
        }

        Candidate[] winners = new Candidate[numtimes * (numOfCandidates - 1)];

        for (int i = 0; i < numtimes; i++) {
            for (int j = 0; j < numOfCandidates; j++) {
                try {
                    winners[(numOfCandidates * i) + j] = election[j].vote(election);
                }
                catch (Exception e) {
                    winners[j] = new Candidate_ec221026();
                }
            }
        }
