...
class Candidate_ec22766 extends Candidate {
    ...
    public static void main(String[] args){
        ...
        while(choice != 'a'){
            ...
            //Option e
                ...
                random2Election(candidates, candidates);
            }
            choice = getChar("\nChoose election type a) no election (exit) " +
                    "b) add a specific candidate " +
                    "d) random 1-election. " +
    ...
    private static void randomElection(Candidate[] possibleCandidates, Citizen[] va) {
        ...
        do {
            ...
            for (int i=0;i<10;i++) {
                ...
                System.out.println("The winner is "+w.getName()+" ("+w.un+"): "+w.getSlogan());
            }
    ...
    public static Candidate[] addCandidate(Candidate[] allCandidates)
    {
        System.out.println("Please enter a username.");
        Scanner sc = new Scanner(System.in);
        String username = sc.nextLine();
        Candidate newCandidate = getByUsername(username, getCandidateArray());
        Candidate[] newArray = new Candidate[allCandidates.length+1];
        for(int i=0; i<allCandidates.length;i++){
            newArray[i] = allCandidates[i];
        }
        newArray[allCandidates.length] = newCandidate;
    ...
    public void runNormalElection(Candidate[] allCandidates){
        Candidate[] candidateArray = getCandidateArray();
        Candidate[] allVotes = new Candidate[candidateArray.length];
        Candidate vote;
        for (int i=0; i<candidateArray.length;i++){
            try {
                allVotes[i] = candidateArray[i].vote(candidateArray);
                System.out.println(allVotes[i]);
            }
        ...
        Candidate winner = selectWinner(allVotes);
        System.out.println("The winner is: "+winner.getName());
        System.out.println("Their slogan is: "+winner.getSlogan());
